	...
	<ul>
		...
		<c:forEach var="a" items="${all}" varStatus="all">
			<li id="cateId" class="menu">
				<div class="t  w3-border-bottom w3-padding-16" value="${a.num }">
					<span id="ex">${a.category }</span>${a.title }</div>
				<div
					class="hide w3-container w3-padding-24 w3-border-bottom w3-light-grey">${a.content }</div>
			</li>
		</c:forEach>
	</ul>
</div>
